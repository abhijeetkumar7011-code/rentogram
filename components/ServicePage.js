...
export default function ServicingPage() {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    // TODO: send servicing request to Supabase `servicing_requests` table
    setSubmitted(true);
  }
      ...
      <div id="request-form" className="bg-white rounded-2xl p-5 sm:p-8 shadow-md border border-gray-100 max-w-2xl mx-auto">
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-primary text-white py-2.5 sm:py-3 rounded-xl font-medium transition-soft hover-lift shadow-md text-sm sm:text-base"
            >
              Submit Request
            </button>
          </form>
